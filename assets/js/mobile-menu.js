...
document.addEventListener('DOMContentLoaded', function () {
    const header = document.querySelector('.site-header.header-area');
    const mediaQuery = window.matchMedia('(max-width: 991px)');
    let headerOffsetTop;

    // Function to check media query and calculate header offset
    function checkStickyCondition() {
        if (mediaQuery.matches) {
            if (header) {
                headerOffsetTop = header.offsetTop; // Get the header's original position
                window.addEventListener('scroll', handleScroll);
            }
        } else {
            if (header) {
                // Remove sticky class and listener for larger screens
    ...
    function handleScroll() {
        if (window.scrollY > headerOffsetTop) {
            ...
            // Remove sticky class if present
            if (header.classList.contains('sticky')) {
                header.classList.remove('sticky');
            }
        }
